...
[ApiController]
[Route("api/administracion/tiempo-respuesta-resolucion")]
[Authorize]
public class AdministracionTiempoRespuestaResolucionController : ControllerBase
{
    private readonly ITiempoRespuestaResolucionService _servicio;
    private readonly IUsuarioContexto _usuarioContexto;

    private const string MODULO = WasionOne.API.Models.Modulos.AdmTiempoRespuestaResolucion;

    public AdministracionTiempoRespuestaResolucionController(ITiempoRespuestaResolucionService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<TiempoRespuestaResolucionDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<TiempoRespuestaResolucionDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<TiempoRespuestaResolucionDto>> CrearRegistro(TiempoRespuestaResolucionCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe un registro de Tiempo de Respuesta y Resolución con ese Folio. Edítalo desde la tabla en vez de crear uno nuevo.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<TiempoRespuestaResolucionDto>> ActualizarRegistro(int id, TiempoRespuestaResolucionActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<TiempoRespuestaResolucionImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-tiempo-respuesta-resolucion.xlsx");
    }
}
