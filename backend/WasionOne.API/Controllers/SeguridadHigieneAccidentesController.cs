...
[ApiController]
[Route("api/seguridad-higiene/accidentes")]
[Authorize]
public class SeguridadHigieneAccidentesController : ControllerBase
{
    private readonly IAccidenteTrabajoService _servicio;
    ...
    private const string MODULO = "seguridadHigiene.accidentes";

    public SeguridadHigieneAccidentesController(IAccidenteTrabajoService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<AccidenteTrabajoDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<AccidenteTrabajoDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<AccidenteTrabajoDto>> CrearRegistro(AccidenteTrabajoCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe un Accidente de trabajo con ese Folio. Edítalo desde la tabla en vez de crear uno nuevo.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<AccidenteTrabajoDto>> ActualizarRegistro(int id, AccidenteTrabajoActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<AccidenteTrabajoImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-accidentes-seg-higiene.xlsx");
    }
}
