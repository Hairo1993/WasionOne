...
[ApiController]
[Route("api/administracion/cumplimiento-programa")]
[Authorize]
public class AdministracionCumplimientoProgramaController : ControllerBase
{
    private readonly ICumplimientoProgramaService _servicio;
    private readonly IUsuarioContexto _usuarioContexto;

    private const string MODULO = WasionOne.API.Models.Modulos.AdmCumplimientoPrograma;

    public AdministracionCumplimientoProgramaController(ICumplimientoProgramaService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<CumplimientoProgramaDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<CumplimientoProgramaDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<CumplimientoProgramaDto>> CrearRegistro(CumplimientoProgramaCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<CumplimientoProgramaDto>> ActualizarRegistro(int id, CumplimientoProgramaActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<CumplimientoProgramaImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-cumplimiento-programa.xlsx");
    }
}
