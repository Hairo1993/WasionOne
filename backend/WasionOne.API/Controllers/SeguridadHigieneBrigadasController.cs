...
// Models/Brigada.cs).
[ApiController]
[Route("api/seguridad-higiene/brigadas")]
[Authorize]
public class SeguridadHigieneBrigadasController : ControllerBase
{
    private readonly IBrigadaService _servicio;
    ...
    // RESUMEN-GRUPO-B.md, constante propuesta Modulos.SegHigBrigadas).
    // Se usa el string literal directamente mientras tanto para no
    // depender de un archivo compartido que no se debe tocar aquí.
    private const string MODULO = "seguridadHigiene.brigadas";

    public SeguridadHigieneBrigadasController(IBrigadaService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<BrigadaDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<BrigadaDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<BrigadaDto>> CrearRegistro(BrigadaCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe un registro de Brigada con ese Folio. Edítalo desde la tabla en vez de crear uno nuevo.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<BrigadaDto>> ActualizarRegistro(int id, BrigadaActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<BrigadaImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-brigadas.xlsx");
    }
}
