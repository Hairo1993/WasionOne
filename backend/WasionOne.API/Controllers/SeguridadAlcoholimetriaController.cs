...
[ApiController]
[Route("api/seguridad/alcoholimetria")]
[Authorize]
public class SeguridadAlcoholimetriaController : ControllerBase
{
    private readonly IAlcoholimetriaService _servicio;
    private readonly IUsuarioContexto _usuarioContexto;

    private const string MODULO = WasionOne.API.Models.Modulos.SegAlcoholimetria;

    public SeguridadAlcoholimetriaController(IAlcoholimetriaService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<AlcoholimetriaDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<AlcoholimetriaDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<AlcoholimetriaDto>> CrearRegistro(AlcoholimetriaCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe un registro de Alcoholimetría para esa Planta, Fecha y Turno. Edítalo desde la tabla en vez de crear uno nuevo.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<AlcoholimetriaDto>> ActualizarRegistro(int id, AlcoholimetriaActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<AlcoholimetriaImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-alcoholimetria.xlsx");
    }
}
