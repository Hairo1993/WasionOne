...
[ApiController]
[Route("api/seguridad-higiene/cumplimiento-epp")]
[Authorize]
public class SeguridadHigieneCumplimientoEppController : ControllerBase
{
    private readonly ICumplimientoEppService _servicio;
    ...
    // RESUMEN-GRUPO-B.md, constante propuesta Modulos.SegHigCumplimientoEpp).
    // Se usa el string literal directamente mientras tanto para no
    // depender de un archivo compartido que no se debe tocar aquí.
    private const string MODULO = "seguridadHigiene.cumplimientoEpp";

    public SeguridadHigieneCumplimientoEppController(ICumplimientoEppService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<CumplimientoEppDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<CumplimientoEppDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<CumplimientoEppDto>> CrearRegistro(CumplimientoEppCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe un registro de Cumplimiento de EPP para ese IdEpp en esa Fecha de registro. Edítalo desde la tabla en vez de crear uno nuevo.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<CumplimientoEppDto>> ActualizarRegistro(int id, CumplimientoEppActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<CumplimientoEppImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-cumplimiento-epp.xlsx");
    }
}
