...
[ApiController]
[Route("api/administracion/cumplimiento-documentacion")]
[Authorize]
public class AdministracionCumplimientoDocumentacionController : ControllerBase
{
    private readonly ICumplimientoDocumentacionService _servicio;
    private readonly IUsuarioContexto _usuarioContexto;

    private const string MODULO = WasionOne.API.Models.Modulos.AdmCumplimientoDocumentacion;

    public AdministracionCumplimientoDocumentacionController(ICumplimientoDocumentacionService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<CumplimientoDocumentacionDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<CumplimientoDocumentacionDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<CumplimientoDocumentacionDto>> CrearRegistro(CumplimientoDocumentacionCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe un registro de Cumplimiento de la Documentación con ese Code. Edítalo desde la tabla en vez de crear uno nuevo.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<CumplimientoDocumentacionDto>> ActualizarRegistro(int id, CumplimientoDocumentacionActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<CumplimientoDocumentacionImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-cumplimiento-documentacion.xlsx");
    }
}
