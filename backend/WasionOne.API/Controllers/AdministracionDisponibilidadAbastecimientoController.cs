...
[ApiController]
[Route("api/administracion/disponibilidad-abastecimiento")]
[Authorize]
public class AdministracionDisponibilidadAbastecimientoController : ControllerBase
{
    private readonly IDisponibilidadAbastecimientoService _servicio;
    private readonly IUsuarioContexto _usuarioContexto;

    private const string MODULO = WasionOne.API.Models.Modulos.AdmDisponibilidadAbastecimiento;

    public AdministracionDisponibilidadAbastecimientoController(IDisponibilidadAbastecimientoService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<DisponibilidadAbastecimientoDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<DisponibilidadAbastecimientoDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<DisponibilidadAbastecimientoDto>> CrearRegistro(DisponibilidadAbastecimientoCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<DisponibilidadAbastecimientoDto>> ActualizarRegistro(int id, DisponibilidadAbastecimientoActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<DisponibilidadAbastecimientoImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-disponibilidad-abastecimiento.xlsx");
    }
}
