...
[ApiController]
[Route("api/seguridad/evaluaciones-vigilancia")]
[Authorize]
public class SeguridadEvaluacionesVigilanciaController : ControllerBase
{
    private readonly IEvaluacionVigilanciaService _servicio;
    private readonly IUsuarioContexto _usuarioContexto;

    private const string MODULO = WasionOne.API.Models.Modulos.SegEvaluacionesVigilancia;

    public SeguridadEvaluacionesVigilanciaController(IEvaluacionVigilanciaService servicio, IUsuarioContexto usuarioContexto)
    {
        _servicio = servicio;
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<EvaluacionVigilanciaDto>>> ObtenerRegistros([FromQuery] int? areaUbicacionId)
    {
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<EvaluacionVigilanciaDto>> ObtenerRegistro(int id)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    [HttpPost]
    public async Task<ActionResult<EvaluacionVigilanciaDto>> CrearRegistro(EvaluacionVigilanciaCrearDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
        ...
        if (registro is null)
        {
            return Conflict("Ya existe una evaluación de vigilancia para esa Planta, Proveedor y Fecha. Edítala desde la tabla en vez de crear una nueva.");
        }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<EvaluacionVigilanciaDto>> ActualizarRegistro(int id, EvaluacionVigilanciaActualizarDto dto)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public async Task<ActionResult<EvaluacionVigilanciaImportarResultadoDto>> ImportarRegistros(IFormFile? archivo)
    {
        if (!AutorizacionModuloHelper.TieneModuloAsignado(_usuarioContexto, MODULO))
        {
            return Forbid();
    ...
    public IActionResult DescargarPlantilla()
    {
        ...
        return File(archivo, ExcelPlantillaUtils.MimeTypeXlsx, "plantilla-evaluaciones-vigilancia.xlsx");
    }
}
